...
<?php foreach ($this->_params['viewAreaTimeslot'] as $value) :?>
                            <option value="<?= $value['startTime']?>" <?= isset($_GET['trainingTime']) ? ($_GET['trainingTime'] === $value ? "selected" : '') : '' ?>><?= timeInRightOrder($value['startTime']).'-'.timeInRightOrder($value['endTime'],true) ?></option>;
                        <?php endforeach; ?>
</select>
